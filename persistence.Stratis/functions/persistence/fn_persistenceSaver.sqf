...
if (isNil "_persistenceData") then {
	if (isNil "_this") then {
		_persistenceData = ["authenticate","Position","Gear","Settings","Group","Vehicle","Medical","Hash"];
	} else {
		_persistenceData = ["authenticate"];
		_persistenceData = _persistenceData + _this;
	};
...
for "_i" from 0 to (count _varNames) -1 do {
	private ["_savingX","_xData","_temp"];
	_temp = "";
	if (isNil {_this select _i}) then {
		_temp = _varNames select _i;
	} else {
		_temp = _this select _i;
	};
	_savingX = format["_saving%1",_temp];
	missionNamespace setVariable [_savingX,_temp];
...
_missionCheck = (_persistenceData select 0);
//Data to save
