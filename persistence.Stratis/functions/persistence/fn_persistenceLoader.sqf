...
if(isNil "_persistenceData") exitWith {
	[_this] spawn RETR_fnc_persistenceSaver;
...
if !(_persistenceData select 0 isEqualTo _authenticationData) exitWith {
	profileNamespace setVariable [_persistenceVarName,nil];
	diag_log format["[Persistence] - %1 loaded invalid Persitance Data", name player];
	[_this] spawn RETR_fnc_persistenceSaver;
};
...
//Compare Hash
if !(_hashDataCompare isEqualTo _hashDataNew) exitWith {
	profileNamespace setVariable [_persistenceVarName,nil];
